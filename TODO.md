...
Laatste sync: 2026-08-31 · branch `main`.
...
- 🟡 **Dagelijkse aanwinsten-push** (30 aug 2026) — job en endpoint staan, **maar er is nog geen scheduler, dus hij is nog nooit afgegaan** (0 van de 65 users heeft een `lastDailyPushAt`). Wat er ligt: `src/jobs/daily-new-push.ts` + `POST /admin/api/push/daily-new` met de admin-bearer, `?dry=1` telt zonder te versturen. Geen in-process cron, want de Fly-machine slaapt tussen requests (`min_machines_running = 0`); zelfde patroon als de scrapers dus. Wat ontbreekt: een trigger om 10:00 Amsterdamse tijd — n8n Schedule-node ligt voor de hand. Drie regels bepalen wie iets krijgt: niks nieuws = geen push, app vandaag al open gehad = geen push, en `lastDailyPushAt` als slot tegen dubbel verzenden. Venster is één dag: met een weekvenster kwam de telling in productie op 373 uit en met de 30-daagse fallback van `/events/new` op 1182. Droog gedraaid: 28 kandidaten, 97 elk.
- ⬜ **Baan-voorkeur naar de server** — `activeLanes` staat in AsyncStorage op het toestel, dus de push telt álle banen. Wie z'n `/new` tot theater beperkt leest straks "97 nieuw" en vindt er vier. Nu geen probleem omdat het filter nauwelijks gebruikt wordt; oplossen door 'm mee te sturen naar `PATCH /me`.
- ✅ **Musea per instelling** (30 aug 2026) — `/musea` als poster-raster van musea in plaats van losse tentoonstellingen, plus dezelfde kaart als rail op Vandaag. Bij film is de film de hoofdzaak en de bioscoop bijzaak; bij een tentoonstelling is het gebouw de constante en wisselt de inhoud. Nieuw endpoint `GET /venues/musea` (12 venues met iets lopends; de 13 galeries zonder programma vallen weg). Titel als accent-blok in het beeld met de einddatum eronder. Verving "Grote kunstmusea" + "Foto & media musea"; "Vandaag te bezoeken" is geschrapt omdat de literatuur-helft één rail verderop stond en losse kunst-events 0–1 per dag zijn.
- ✅ **Hero op Vandaag** (30 aug 2026) — één redactionele keuze plus twee uit `/for-you`, zeven dagen horizon, kicker per slide. Was een cascade die meestal op één kaart uitkwam.
- ✅ **Rails heten naar hun categorie** (30 aug 2026) — "Vannacht in de clubs" / "Film vanavond" beloofden een tijdsframe dat de pagina niet maakt: `mode` wordt op Vandaag nergens gelezen en alle rails renderen altijd. Het `'uit'`/`'expo'`-gating uit de comments bestaat niet meer. Thema en de dag/nacht-schakelaar blijven; die stuurt alleen geen inhoud aan.
